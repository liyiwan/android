...
public class MainHomeFragment extends Fragment implements OnClickListener {
    ...
    private TextView tv_productsearch;
    private TabPagerAdapter mPagerAdapter;
    /*** 当前页卡编号 ***/
    private int currIndex = 0;
    /*** 主题选择按钮 ***/
    @ViewInject(R.id.tv_mainhome_iwusetheme)
    private TextView tv_mainhome_iwusetheme;
    /*** 单品选择按钮 ***/
    @ViewInject(R.id.tv_mainhome_productlist)
    private TextView tv_mainhome_productlist;
	...
	// ll_top_left_menu = (LinearLayout)
	// mView.findViewById(R.id.ll_top_menu);
	...
	public Fragment getItem(int position) {
	    ...
	    switch (position) {
	    case 0:
		fragment = (WuseThemeFragment) Fragment.instantiate(
			getActivity(), WuseThemeFragment.class.getName());
		break;
	    case 1:
		fragment = (ProductListFragment) Fragment.instantiate(
			getActivity(), ProductListFragment.class.getName());
		break;
	    }
	    return fragment;
	}
	...
	case R.id.tv_productsearch:
		...
		FragmentTransaction transaction = mFragmentManager
			.beginTransaction();
		transaction.setCustomAnimations(R.anim.push_top_in,
			R.anim.push_top_out);
		isFilterOpen = true;
		...
		if (currentItem == 0) {
			...
			transaction.add(R.id.fl_filter_fragment,
				productFragment);
		    } else {
